...
impl<'b, T: Clone + Copy + PartialEq + std::fmt::Debug + 'static> Bucket<T> {
    pub(crate) fn new(
        ...
        let (index, random, reused_file_at_startup) = reuse_path
            .and_then(|path| {
                // try to reused the file this bucket was using last time we were running
                restartable_bucket.get().and_then(|(_file_name, random)| {
                    ...
                    if result.is_none() {
                        // we couldn't reused it, so delete it
            ...
            .unwrap_or_else(|| {
                // no file to reused, so create a new file
