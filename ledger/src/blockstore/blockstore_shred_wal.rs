...
use {
    super::*,
    crate::shred::{ShredId, SHRED_PAYLOAD_SIZE},
    std::{
        cmp,
...
impl ShredWAL {
    ...
    pub fn log_shred_write(&mut self, mut shreds: HashMap<ShredId, Shred>) -> Result<()> {
        let mut log = self.open_log(shreds.len())?;
        let mut insert_max_slot = 0;
        let result: Result<Vec<_>> = shreds
            .iter_mut()
            .map(|(_shred_id, shred)| {
                insert_max_slot = cmp::max(insert_max_slot, shred.slot());
...
pub mod tests {
    ...
    fn test_wal_write_and_recover() {
        ...
        for _ in 0..num_slots {
            let write_shreds: HashMap<ShredId, Shred> = shreds
                .drain(0..shreds_per_slot)
                .collect::<Vec<_>>()
                .into_iter()
                .map(|shred| (shred.id(), shred))
                .collect();
    ...
    fn test_wal_purge() {
        ...
        let (mut shreds, _) = make_many_slot_entries(0, num_slots, entries_per_slot);
        // Keep a copy of shreds to check against at end
        let all_shreds: HashMap<ShredId, Shred> = shreds
            .iter()
            .cloned()
            .map(|shred| (shred.id(), shred))
            .collect();
        ...
        // Read the shreds back from logs and ensure everything we inserted is present
        let recoverd_shreds = test_flatten_recovered_shreds(shred_wal.recover().unwrap());
        for (shred_id, shred) in all_shreds.iter() {
            let (slot, index, _shred_type) = shred_id.unwrap();
            match slot {
                4 | 5 | 9..=13 => {
                    assert_eq!(shred, recoverd_shreds.get(&(slot, index.into())).unwrap());
                }
                _ => {
                    assert!(recoverd_shreds.get(&(slot, index.into())).is_none());
                }
            }
        }
    }
