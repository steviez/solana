...
pub fn ed25519_verify_disabled(batches: &mut [PacketBatch]) {
    ...
    batches.iter_mut().flatten().for_each(|mut packet| {
...
mod tests {
    ...
    fn ed25519_verify(batches: &mut [PacketBatch]) {
        // Four threads is plenty for unit tests; any unit test that needs more
        // is generating too many packets and should make less or be a bench
